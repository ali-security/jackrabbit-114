...
public class ParserTest extends TestCase {
    ...
    public void testBillionLaughs() throws UnsupportedEncodingException {
        ...
        String testBody = "<?xml version=\"1.0\"?>" + "<!DOCTYPE lolz [" + " <!ENTITY lol \"lol\">"
                + " <!ELEMENT lolz (#PCDATA)>"
    ...
    public void testExternalEntities() throws IOException {
        ...
        String testBody = "<?xml version='1.0'?>\n<!DOCTYPE foo [" + " <!ENTITY test SYSTEM \"file:" + dname + "/"
                + fname + "\">"
                + "]>\n<foo>&test;</foo>";
        InputStream is = new ByteArrayInputStream(testBody.getBytes("UTF-8"));
        ...
        try {
            ...
            fail("parsing this document should cause an exception, but the following external content was included: "
                    + text);
        } catch (Exception expected) {
        }
    }
